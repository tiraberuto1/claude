...
class TranslationPipeline {
    ...
    /** 文字を読み取り、吹き出し単位にまとめる */
    suspend fun detect(bitmap: Bitmap): List<Balloon> = grouper.group(ocr(bitmap))

    /** 吹き出しを覆う位置と色（訳文は空）。一括処理では画像を捨てる前にこれだけ取っておく */
    fun geometry(bitmap: Bitmap, balloons: List<Balloon>): Map<Int, Patch> = balloons.associate { it.id to patchFor(bitmap, it, "") }

    suspend fun run(bitmap: Bitmap, translator: Translator): List<Patch> {
        val balloons = detect(bitmap)
    ...
    /** 絵も見る翻訳器に渡す画像。長辺 2000px に縮めて JPEG にする */
